...
class OrderCartService
{
    ...
    public function tableSelectionData(string $search = ''): array
    {
        ...
        $tablesByStatus = $tables->groupBy(fn (Table $table) => $table->status ?: '__unassigned__');
    ...
    public function findAvailableTable(string $tableId): ?Table
    {
        ...
        return TableStatus::tryFrom((string) $table->status)?->isFree() ? $table : null;
    }
    ...
    public function findOrderableTable(string $tableId): ?Table
    {
        ...
        $status = TableStatus::tryFrom((string) $table->status);

        return $status?->isOrderable() ? $table : null;
    }
    ...
    public function placeOrder(string $tableId, ?string $notes = null): Order
    {
        ...
        $order = DB::transaction(function () use ($table, $cart, $notes): Order {
