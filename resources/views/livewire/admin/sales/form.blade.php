    ...
    <form wire:submit="save" class="space-y-5">
        <div class="grid gap-4 rounded-2xl border border-stone-200 bg-white p-4 md:grid-cols-3 md:p-5">
            <fieldset class="fieldset">
                <legend class="fieldset-legend">Pelanggan</legend>
                <select class="select select-bordered w-full" wire:model.defer="customer_id" @disabled($posted)>
            ...
            <fieldset class="fieldset">
                ...
                <input type="date" class="input input-bordered w-full" wire:model.defer="sale_date" @disabled($posted)>
                @error('sale_date')
                    <p class="label text-error">{{ $message }}</p>
                @enderror
            </fieldset>
            ...
            <div class="flex gap-2">
                ...
                @unless ($posted)
                    ...
                    <button type="button" class="btn bg-emerald-800 text-amber-50 hover:bg-emerald-700"
                        ...
                        wire:click="post">
                        <i class="ri-check-double-line"></i> Posting & Kurangi Stok
                    </button>
                @endunless
            </div>
        </div>
