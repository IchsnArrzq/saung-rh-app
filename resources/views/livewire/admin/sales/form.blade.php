<div class="space-y-5">
    ...
    @php($posted = $sale && $sale->status === 'posted')

    <form wire:submit="save" class="space-y-5">
        <x-card>
            <div class="grid gap-4 md:grid-cols-3">
                <x-select label="Pelanggan" name="customer_id" placeholder="Tanpa pelanggan"
                    wire:model.defer="customer_id" :disabled="$posted">
                    @foreach ($customers as $customer)
                        <option value="{{ $customer->id }}">{{ $customer->name }}</option>
                    @endforeach
                </x-select>

                <x-input label="Tanggal" name="sale_date" type="date"
                    wire:model.defer="sale_date" :disabled="$posted" />
        ...
        <x-data-table :zebra="false">
            ...
            @forelse ($items as $index => $row)
                @php($subtotal = (float) ($row['qty'] ?: 0) * (float) ($row['unit_price'] ?: 0))
                @php($total += $subtotal)
                <tr wire:key="sale-item-{{ $index }}">
                    ...
                    <td>
                        <x-input :bare="true" type="number" size="sm" step="0.01" min="0"
                            ...
                            wire:model.live.debounce.400ms="items.{{ $index }}.unit_price" :disabled="$posted" />
        ...
        <div class="flex flex-wrap items-center justify-between gap-2">
            ...
            <div class="flex gap-2">
                <x-button variant="ghost" :href="route('sales.index')">Kembali</x-button>
                ...
                @unless ($posted)
                    ...
                    <x-button variant="primary" icon="ri-check-double-line" wire:click="post" loading="post"
                        data-confirm="Posting penjualan? Stok bahan akan dikurangi dan dokumen tidak bisa diubah lagi."
                        data-confirm-title="Posting Penjualan" data-confirm-yes="Ya, Posting">
