                ...
                @forelse($customers as $customer)
                    <tr>
                        ...
                        <td class="text-center">
                            <form action="{{ route('customer-users.status', $customer) }}" method="POST"
                                class="inline">
                                @csrf
                                @method('PATCH')
                                <button type="submit"
                                    class="badge {{ $customer->is_active ? 'badge-success' : 'badge-error' }} badge-outline hover:opacity-80">
                        ...
                        <td class="text-right">
                            <div class="inline-flex gap-2">
                                <a href="{{ route('customer-users.edit', $customer) }}"
